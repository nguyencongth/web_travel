...
if(isset($_POST['submitup']))
{
  ...
  $con ="SELECT EmailId FROM tblusers WHERE EmailId=:email";
  $check = $dbh -> prepare($con);
  $check-> bindParam(':email', $email, PDO::PARAM_STR);
  $check-> execute();
  if (filter_var($email, FILTER_VALIDATE_EMAIL)===false || $check -> rowCount() > 0 || empty($email)) {
    $_SESSION['msgersu']="Email đã trùng trong cơ sở dữ liệu hoặc bạn điền sai định dạng email";
    ...
    if($lastInsertId)
    {
      $_SESSION['msgok']="You are Successfully registered. Now you can login OKE NHA ";
